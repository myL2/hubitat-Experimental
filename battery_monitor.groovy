...
def formatTimeAgo(ts){
    ...
    def hours = (mins / 60).toInteger()
    if (mins < 60) return "${mins}m ago"
    if (hours < 48) return "${hours}h ago"
    return "${(hours / 24).toInteger()}d ago"
}
...
def getActivityDisplay(device){
    def ts = getLastActivityTime(device)
    if(!ts) return "N/A"
    def mins = ((now() - ts) / (1000*60)).toInteger()
    def label = formatTimeAgo(ts)
    if(mins < 1440) return "🟢 ${label}"
    if(mins < 10080) return "🟠 ${label}"
    return "🔴 ${label}"
}

def isActivityRed(device){
    def ts = getLastActivityTime(device)
    if(!ts) return true
    def mins = ((now() - ts) / (1000*60)).toInteger()
    return mins >= 10080
    ...
    def hubsJson = groups.collectEntries { hubName, hubDevs ->
        [(hubName): hubDevs.collect { device ->
            ...
            [
                ...
                drain       : (getDrain(device) as BigDecimal).setScale(2, java.math.RoundingMode.HALF_UP),
